 ...
 */
const getProjectTemplate = async (options) => {
 ...
 */
const showTemplateSelect = async (projectTemplate, templates, options) => {
  if (!projectTemplate) {
    options.template && console.log(chalk.red("指定模版找不到！"));
...
const createTemplete = async (projectName, options) => {
  let { templates, projectTemplate } = await getProjectTemplate(options);
  projectName = (await showProjectNameInput(projectName)).projectName;
  projectTemplate = (
    await showTemplateSelect(projectTemplate, templates, options)
  ).projectTemplate;
